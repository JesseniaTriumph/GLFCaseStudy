...
const BRAND = "#df4329"; // GitLab Foundation orange-red
...
function svg(size: number, pad: number, bg: string | null): string {
  ...
  <g transform="translate(${off} ${off}) scale(${scale})">
    <path d="${leaf}" fill="${bg ? "#fff" : BRAND}"/>
    <circle cx="15" cy="15" r="2.4" fill="${bg ?? "#fff"}" stroke="${bg ? "#fff" : BRAND}" stroke-width="1.3"/>
  </g>
</svg>`;
}

const jobs: [string, string][] = [
  ["icon-192.png", svg(192, 0.14, BRAND)],
  ["icon-512.png", svg(512, 0.14, BRAND)],
  ["icon-maskable-512.png", svg(512, 0.26, BRAND)], // extra margin for the maskable safe zone
  ["apple-touch-icon.png", svg(180, 0.12, BRAND)],
  ["favicon-32.png", svg(32, 0.08, BRAND)],
];
